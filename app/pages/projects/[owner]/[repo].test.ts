...
const { sanitizeMarkdownSpy, extractHeadingsSpy } = vi.hoisted(() => ({
  sanitizeMarkdownSpy: vi.fn().mockResolvedValue("<p>processed</p>"),
  extractHeadingsSpy: vi.fn().mockReturnValue([]),
}));

vi.mock("~/composables/useMarkdown", () => ({
  useMarkdown: () => ({
    sanitizeMarkdown: sanitizeMarkdownSpy,
    extractHeadings: extractHeadingsSpy,
...
describe("pages/projects/[owner]/[repo].vue", () => {
  ...
  it("renders loading state initially", async () => {
    ...
    expect(
      wrapper.find('output[aria-label="Loading repository"]').exists(),
    ).toBe(true);
  ...
  it("renders repository details when loaded", async () => {
    ...
    expect(
      wrapper.find('a[href="https://github.com/owner/repo"]').exists(),
    ).toBe(true);
  });
  ...
  it("passes imageBaseUrl with defaultBranch to sanitizeMarkdown", async () => {
    sanitizeMarkdownSpy.mockResolvedValue("<p>processed</p>");
    mockFetch.mockResolvedValue({ ...mockDetail, defaultBranch: "master" });
    await mountSuspended(RepoPage, { route: "/projects/owner/repo" });
    await flushPromises();
    await nextTick();

    expect(sanitizeMarkdownSpy).toHaveBeenCalledWith(
      mockDetail.readmeExcerpt,
      "https://raw.githubusercontent.com/owner/repo/master",
    );
  });

  it("falls back to main branch in imageBaseUrl when defaultBranch absent", async () => {
    sanitizeMarkdownSpy.mockResolvedValue("<p>processed</p>");
