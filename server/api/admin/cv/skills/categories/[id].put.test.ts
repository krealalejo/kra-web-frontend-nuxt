...
describe('server/api/admin/cv/skills/categories/[id].put', () => {
  beforeEach(() => {
    vi.clearAllMocks()
    vi.stubGlobal('useRuntimeConfig', vi.fn().mockReturnValue({ apiBase: 'http://localhost:3001', public: { apiBase: 'http://localhost:3001' } }))
    vi.stubGlobal('readBody', vi.fn().mockResolvedValue({
      name: 'Backend', skills: ['Java', 'Spring Boot', 'Kotlin'],
    }))
  })
  ...
  it('calls PUT on the upstream API with auth header and forwards skills array', async () => {
    const updated = { id: 'cat-1', name: 'Backend', skills: ['Java', 'Spring Boot', 'Kotlin'] }
    ...
    expect(mockFetch).toHaveBeenCalledWith(
      expect.stringContaining('/cv/skills/categories/cat-1'),
    ...
    expect(calledBody).toHaveProperty('skills')
    expect(result).toEqual(updated)
  })

  it('does NOT include sortOrder in forwarded body', async () => {
    vi.stubGlobal('readBody', vi.fn().mockResolvedValue({
      name: 'Backend', skills: ['Java'], sortOrder: 99,
    }))
